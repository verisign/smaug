...
const PGPMIME_JS_ENCRYPT_CONTRACTID = "@smaug.net/smaug/composesecure;1";
...
PgpMimeEncrypt.prototype = {
  classDescription: "Smaug JS Encryption Handler",
  classID: PGPMIME_JS_ENCRYPT_CID,
  contractID: PGPMIME_JS_ENCRYPT_CONTRACTID,
  QueryInterface: XPCOMUtils.generateQI([Ci.nsIMsgComposeSecure, Ci.nsIStreamListener, Ci.nsISmgScriptableMsgCompose]),
  ...
  requiresCryptoEncapsulation: function (msgIdentity, msgCompFields) {
    ...
    try {
      ...
      try {
        var smgSecurityInfo = securityInfo.QueryInterface(Ci.nsISmgMsgCompFields);
        return (smgSecurityInfo.sendFlags & (Ci.nsIEnigmail.SEND_SIGNED | Ci.nsIEnigmail.SEND_ENCRYPTED)) != 0;
      }
      catch (ex) {
        return false;
      }
  ...
  beginCryptoEncapsulation: function (outStream, recipientList, msgCompFields, msgIdentity, sendReport, isDraft) {
    ...
    try {
      ...
      this.smgSecurityInfo = securityInfo.QueryInterface(Ci.nsISmgMsgCompFields); //might throw an error
      ...
      if (securityInfo.sendFlags & Ci.nsIEnigmail.SEND_PGP_MIME) {
        ...
        if (securityInfo.sendFlags & Ci.nsIEnigmail.SEND_ENCRYPTED) {
          ...
          if (Ec.determineHashAlgorithm(this.win,
                    this.smgSecurityInfo.UIFlags,
                    this.smgSecurityInfo.senderEmailAddr,
                    hashAlgoObj) == 0) {
      ...
      this.proc = Ec.encryptMessageStart(this.win,
                      this.smgSecurityInfo.UIFlags,
                      this.smgSecurityInfo.senderEmailAddr,
                      this.smgSecurityInfo.recipients,
                      this.smgSecurityInfo.bccRecipients,
                      this.hashAlgorithm,
                      this.smgSecurityInfo.sendFlags,
                      this,
                      statusFlagsObj,
                      errorMsgObj);
      ...
      if (this.inputMode == 0) {
        ...
        if (line.replace(/[\r\n]/g, "").length == 0) {
          ...
          if (this.cryptoMode == MIME_ENCRYPTED) {
            ...
            let hdr = SmaugFuncs.getHeaderData(ct);
