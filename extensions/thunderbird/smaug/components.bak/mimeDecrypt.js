...
PgpMimeDecrypt.prototype = {
  classDescription: "Smaug JS Decryption Handler",
  ...
  onStartRequest: function(request, uri) {
    if (!Ec.getService()) // Ensure Smaug is initialized
      return;
    ...
    this.exitCode = null;
    this.msgWindow = SmaugVerify.lastMsgWindow;
    this.msgUriSpec = SmaugVerify.lastMsgUri;
    this.verifier = null;
    ...
    this.verifier = SmaugVerify.newVerifier(true, undefined, false);
  ...
  displayStatus: function() {
    ...
    try {
      ...
      let headerSink = this.msgWindow.msgHeaderSink.securityInfo.QueryInterface(Ci.nsISmgMimeHeaderSink);
    ...
    if (i > 0) {
      ...
      for (j in hdr) {
        if (hdr[j].search(/^\s*content-type:\s+text\/(plain|html)/i) >= 0) {
          ...
          this.decryptedData = 'Content-Type: multipart/mixed; boundary="smaugWrapper"\r\n\r\n'+
            '--smaugWrapper\r\n' +
            this.decryptedData +
            '--smaugWrapper--\r\n';
          break;
